...
This fork also contains a production-oriented Android edition under [`android/`](android/). It is designed for Samsung Galaxy phones with S Pen support and includes full-screen parchment writing, on-device LiteRT-LM models, OpenAI-compatible APIs, conversation history, local memory, custom instruction files, diagnostics, and background model downloads.

Build it with Android Studio by opening the `android` directory, or use the Android workflow artifacts generated by this repository. The original Rust/reMarkable implementation remains intact. Android development and releases happen only in this fork; no pull request is submitted to the upstream reMarkable project.
...
This repository is a fork of MaximeRivest/riddle; the Android edition is maintained here independently and is not submitted upstream.
